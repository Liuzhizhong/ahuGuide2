...
public class AhuGuideAlgorithm {
	 ...
	 * @param end	结束点序号
	 * @param count	点的个数
	 * @param map	图  图中key为序号，value为节点的坐标以及与该节点相关节点，以及距离
	 * @return
	 */
	public static Integer[] Dijkstra(int start,int end,int count,Map<Integer,Vertex> map)//迪杰斯特拉算法求单源节点最短路径
	{
		if (start == end)//如果起始节点和终止节点相同则为空
		{
			return null;
		}
...
//		===============================================
		...
		/**
		 * Dijkstra算法也是基于贪心算法策略。双重循环，外层循环内有两个并列的单层循环，任取一个循环内的操作为基本操作，
		 * 则基本操作执行的总次数为双重循环执行的次数。
		 * 
		 */
